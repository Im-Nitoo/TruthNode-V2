...
import { verifyToken, verifyTokenDirect, generateToken } from './auth';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const authenticateUser = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const decoded = await verifyTokenDirect(token);
      if (!decoded) {
  ...
  app.post('/api/evidence', authenticateUser, async (req, res) => {
    try {
      ...
      const parsedArticleId = typeof evidenceData.articleId === 'string' 
        ? parseInt(evidenceData.articleId, 10)
        : Number(evidenceData.articleId);
        
      if (isNaN(parsedArticleId)) {
        return res.status(400).json({ message: "Invalid article ID" });
      }
      
      const article = await storage.getArticle(parsedArticleId);
      if (!article) {
        return res.status(404).json({ message: "Article not found" });
      }
  ...
  app.post('/api/verifications', authenticateUser, async (req, res) => {
    try {
      ...
      const parsedArticleId = typeof verificationData.articleId === 'string'
        ? parseInt(verificationData.articleId, 10)
        : Number(verificationData.articleId);
  ...
  httpServer.on('upgrade', async (request: IncomingMessage, socket: Socket, head: Buffer) => {
    try {
      ...
      // Only handle WebSocket connections on the /ws path
      if (url.pathname !== '/ws') {
      ...
      const decoded = await verifyTokenDirect(decodedToken);
      if (!decoded) {
        socket.write('HTTP/1.1 401 Unauthorized\r\n\r\n');
        socket.destroy();
        return;
