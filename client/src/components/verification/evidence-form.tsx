...
export function EvidenceForm({ articles = [], preselectedArticleId }: EvidenceFormProps) {
  ...
  const onSubmit = async (data: z.infer<typeof evidenceSchema>) => {
    ...
    if (!token) {
      toast({
        title: "Authentication required",
        description: "Please log in again to submit evidence",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (uploadedFiles.length > 0) {
        ...
        if (result) {
          ...
          throw new Error("Failed to upload file to IPFS: CID is undefined");
        }
      }

      // Submit the evidence with the Authorization header
      console.log("Submitting evidence with token:", token); // debugging line
